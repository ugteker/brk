...
| Manual episode-picker run always shows "no content", but works fine locally | YouTube blocks/rate-limits unauthenticated caption/transcript requests from known datacenter/VPS IP ranges (Hetzner included) more aggressively than residential IPs. The app already impersonates several browser/app clients with realistic headers to reduce this, but if a specific episode still fails only from the server, set `YOUTUBE_PROXY_URL` in `.env` to a residential/mobile proxy service URL (e.g. `http://user:pass@host:port`) — this routes just the YouTube caption requests through a non-datacenter IP and reliably bypasses the block. Check the Runs view: the warning message now always includes the failing episode's clickable URL. |
